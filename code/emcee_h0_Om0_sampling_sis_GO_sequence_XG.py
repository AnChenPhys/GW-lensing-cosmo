...
def log_likelihood(free_param,dL_mean,delta_dL,t_delay_obs,zL,zS,thetaE,yh,delta_yh):
    H0 = free_param[0]
    Om0 = free_param[1]
    ...
    if 10<=H0<=200 and 0.01<=Om0<=0.99:
        ...
        loglik += -0.5 * np.sum((dL_mean - cosmo.luminosity_distance(zS).value)**2/delta_dL**2)

        t_delay_thy = t_delay(H0,Om0,zL,zS,thetaE,yh)
        delta_t_delay = t_delay(H0,Om0,zL,zS,thetaE,delta_yh)
        loglik += -0.5 * np.sum((t_delay_obs - t_delay_thy)**2/delta_t_delay**2)
    else:
        loglik = -np.inf
...
for i in range(2):#len(ind_dict['seed'])):
...
#         break
    ...
    error = np.loadtxt('fisher_error_XG/fisher_error_XG_%d.txt'%i)
    delta_dL = error[:,2]
    delta_yh = error[:,4]
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_likelihood, args=[dL_mean, delta_dL, t_delay_obs, zL_ind,zS_ind,thetaE,y_ind,delta_yh])
    ...
    np.savetxt(f'../emcee_samples_y_XG/emcee_H0_Om0_seed_{seed}_samples_XG.txt',samples)
    np.savetxt('event_ind_seed_XG.txt',seed_arr)
    ...
    plt.savefig(f'../plots_emcee_y_XG/emcee_H0_Om0_seed_{seed}_XG.png')
