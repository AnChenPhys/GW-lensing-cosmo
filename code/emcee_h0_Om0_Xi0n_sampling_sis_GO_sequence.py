...
def dGW_dL_ratio_Xi0n(z,Xi0,n):
    return Xi0 + (1-Xi0) / np.power(1+z,n)
...
def log_likelihood(free_param,dL_mean,dL_std,t_delay_obs,sigma_t_sq,zL,zS,thetaE,yh):
    ...
    loglik = 0
    if 10<=H0<=200 and 0.01<=Om0<=0.99 and 0.1<=Xi0<=10 and 0<=n<=10:
        cosmo = FlatLambdaCDM(H0=H0,Om0=Om0)
        for i in range(len(dL_mean)):
            dGW_dL_ratio = dGW_dL_ratio_Xi0n(zS[i],Xi0,n)
...
# for i in range(n*90,(n+1)*90):
    ...
    nwalkers = 32
    ndim = 4
    p0 = np.random.rand(nwalkers, ndim)*np.array([190,0.9,9.9,10]) + np.array([10,0.05,0.1,0])
    ...
    np.savetxt(f'../emcee_samples/emcee_H0_Om0_Xi0n_seed_{seed}_samples.txt',samples)
    ...
    plt.savefig(f'../plots/emcee_H0_Om0_Xi0n_seed_{seed}.png')
