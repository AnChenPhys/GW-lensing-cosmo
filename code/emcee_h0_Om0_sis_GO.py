...
# lik = np.zeros((len(h0_arr),len(Om0_arr)))
# for i in range(len(h0_arr)):
#     for j in range(len(Om0_arr)):
#         lik[i,j] = Loglik([h0_arr[i],Om0_arr[j]], dL_mean, dL_std, t_delay_obs, sigma_t_sq,zL,zS,thetaE,yh)
# np.savetxt('H0_Om0_loglik_seed_0_ind_5081.txt',lik)
...
#             ax[row,column].set_ylim(0.05,0.95)
#             ax[row,column].xaxis.set_tick_params(labelsize=15)
#             ax[row,column].yaxis.set_tick_params(labelsize=15)
...
# plt.savefig('../plots/lik_h0_Om0.png')
...
nwalkers = 32
ndim = 2
p0 = np.random.rand(nwalkers, ndim)*np.array([1.2,0.9]) + np.array([0.2,0.05])
...
np.savetxt('emcee_H0_Om0_seed_0_ind_5081_samples.txt',samples)
...
plt.savefig('../plots/emcee_H0_Om0_seed_0_ind_5081.png')
