    ...
    def log_likelihood(self):
        ...
        if self.parameters['m1']<self.parameters['m2']:
            ...
            for det in self.dets:
                ...
                F_h0_plus = np.sqrt(np.abs(mu_plus(self.parameters['y']))) * (Fp_plus*h0['plus']+Fx_plus*h0['cross']) * np.exp(-2.j*np.pi*freq0*tc_scale)
                F_h0_minus = np.sqrt(np.abs(mu_minus(self.parameters['y']))) * (Fp_minus*h0['plus']+Fx_minus*h0['cross']) * np.exp(-2.j*np.pi*freq0*tc_scale-1.j*(np.pi/2.))
...
result=bilby.run_sampler(likelihood, priors, sampler='nessai',nlive=700,naccept=60, check_point_delta_t=1800, print_method='interval-60', 
                         sample='acceptance-walk', npool=16,outdir='./results_sis_GO/',allow_multi_valued_likelihood = True)
