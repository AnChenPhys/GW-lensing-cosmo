...
# seed_list = np.loadtxt('event_ind_seed_Xi0n.txt')
...
Xi0_arr = np.linspace(0.1,10,100)
n_arr = np.linspace(0.,10,100)
param_values = [H0_arr,Om0_arr,Xi0_arr,n_arr]

H0_arr_2d = np.linspace(20,140,100)
Om0_arr_2d = np.linspace(0.01,0.99,100)
param_values_2d = [H0_arr_2d,Om0_arr_2d,Xi0_arr,n_arr]
...
H0_arr_XG_2d = np.linspace(50,90,100)
Om0_arr_XG_2d = np.linspace(0.01,0.99,100)
Xi0_arr_XG_2d = np.linspace(0.1,6,100)
n_arr_XG_2d = np.linspace(0.,10,100)
param_values_XG_2d = [H0_arr_XG_2d,Om0_arr_XG_2d,Xi0_arr_XG_2d,n_arr_XG_2d]

H0_lim_low, H0_lim_high = 40, 100
Om0_lim_low, Om0_lim_high = 0.01, 0.9
Xi0_lim_low, Xi0_lim_high = 0.1, 3.2
n_lim_low, n_lim_high = 0, 8
lim_low = [H0_lim_low,Om0_lim_low,Xi0_lim_low,n_lim_low]
lim_high = [H0_lim_high,Om0_lim_high,Xi0_lim_high,n_lim_high]

ndim = 4
label = [r'$H_0$',r'$\Omega_{m0}$',r'$\Xi_0$',r'$n$']
...
for column in np.arange(0,ndim):
    for row in np.arange(0,ndim):
        ...
        if column > row:
            ...
            #     emcee_samples = np.loadtxt(f'../emcee_samples_new/emcee_H0_Om0_Xi0n_seed_{seed}_samples.txt')

            #     p1d_kde = gaussian_kde(emcee_samples[:,row])
            #     ind_lik_norm_avg += p1d_kde(param_values[row]) / len(seed_list)

            # np.savetxt(f'average_lik_Xi0n_1d_{row}_new.txt',ind_lik_norm_avg)

            ind_lik_norm_avg = np.loadtxt(f'average_lik_y_data/average_lik_Xi0n_1d_{row}.txt')
            ind_lik_norm_avg_O6 = np.loadtxt(f'average_lik_y_data/average_lik_Xi0n_1d_{row}_O6.txt')
            ind_lik_norm_avg_XG = np.loadtxt(f'average_lik_y_data/average_lik_Xi0n_1d_{row}_XG.txt')
            ...
            if row != 3:
                ...
                ax[row,column].set_xticks([2,4,6])
            ...
            # np.savetxt(f'average_lik_Xi0n_2d_{row}_{column}_new.txt',comb_likelihood_avg)

            # comb_likelihood_avg = np.loadtxt(f'average_lik_Xi0n_2d_{row}_{column}_new.txt')
            
            comb_likelihood_avg = np.loadtxt(f'average_lik_y_data/average_lik_Xi0n_2d_{row}_{column}.txt')
            comb_likelihood_avg_O6 = np.loadtxt(f'average_lik_y_data/average_lik_Xi0n_2d_{row}_{column}_O6.txt')
            comb_likelihood_avg_XG = np.loadtxt(f'average_lik_y_data/average_lik_Xi0n_2d_{row}_{column}_XG.txt')
        ...
        ax[3,2].set_xticks([1,2,3])
        ax[1,0].set_yticks([0.2,0.5,0.8])
        ax[3,0].set_xticks([50,70,90])
        ax[2,0].set_yticks([1,2,3])
        ax[3,0].set_yticks([2,4,6])
        if row != 3:
            ax[row,column].set_xticks([])
        if column != 0:
            ax[row,column].set_yticks([])
            ...
            # ax[2,2].set_xlabel(label[2], fontsize=16)
            # ax[2,2].set_ylabel('p('+label[2]+')', fontsize=16)
            # ax[2,2].yaxis.set_label_position("right")
            # ax[2,2].yaxis.tick_right()
            # ax[2,0].set_xlabel(label[0], fontsize=16)
            # ax[2,0].set_ylabel(label[2], fontsize=16)
            # ax[2,1].set_xlabel(label[1], fontsize=16)
...
plt.savefig(f'../plots/emcee_h0_Om0_Xi0n_combine_averaged.png',dpi=600)
