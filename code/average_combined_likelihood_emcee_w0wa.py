...
# seed_list = np.loadtxt('event_ind_seed_XG.txt')
...
# for i in range(3,4):
#     ind_lik_norm_avg = 0
#     for s in range(27):#(len(ind_dict['seed'])):
#         seed = ind_dict['seed'][s]
#         emcee_samples = np.loadtxt(f'../emcee_samples_y_XG/emcee_H0_Om0_w0wa_seed_{seed}_samples_y_XG.txt')

#         p1d_kde = gaussian_kde(emcee_samples[:,i],bw_method=0.5)
...
for column in np.arange(0,ndim):
    for row in np.arange(0,ndim):
        ...
        if column > row:
            ...
            # ind_lik_norm_avg_XG = 0
            # for i in range(len(seed_list)):
                # seed = int(seed_list[i])
                # emcee_samples = np.loadtxt(f'../emcee_samples_y_XG/emcee_H0_Om0_w0wa_seed_{i}_samples_y_XG.txt')

                # p1d_kde = gaussian_kde(emcee_samples[:,row],bw_method=0.5)
                # ind_lik_norm_avg_XG += p1d_kde(param_values_XG[row]) #/ len(seed_list)
            ...
            ind_lik_norm_avg_XG = np.loadtxt(f'average_lik_y_data/average_lik_w0wa_1d_{row}_XG.txt')
            ...
            # comb_likelihood_avg_XG = 0
            # for i in range(1):#(len(seed_list)):
                # seed = int(seed_list[i])
                # emcee_samples = np.loadtxt(f'../emcee_samples_y_XG/emcee_H0_Om0_w0wa_seed_{i}_samples_y_XG.txt')
                # p2d_kde = gaussian_kde([emcee_samples.T[row],emcee_samples.T[column]],bw_method=0.5)

                # p2d = np.zeros((len(param_values_XG_2d[row]),len(param_values_XG_2d[column])))
                # for j in range(len(param_values_XG_2d[row])):
                #     for k in range(len(param_values_XG_2d[column])):
                #         p2d[j,k] = p2d_kde((param_values_XG_2d[row][j], param_values_XG_2d[column][k]))
                # comb_likelihood_avg_XG += p2d #/ len(seed_list)
            ...
            comb_likelihood_avg_XG = np.loadtxt(f'average_lik_y_data/average_lik_w0wa_2d_{row}_{column}_XG.txt')
...
plt.savefig(f'../plots/emcee_h0_Om0_w0wa_combine_averaged.png',dpi=600)
