...
for i in range(n*90,(n+1)*90):
    ...
    for n in range(len(lensed_index)):
        ...
        tc_diff[n] = np.mean(tc_minus) - np.mean(tc_plus)
        sigma_t_sq[n] = np.std(tc_plus)**2 + np.std(tc_minus)**2
    ...
    np.savetxt(f'emcee_H0_Om0_seed_{seed}_samples.txt',samples)
    ...
    plt.savefig(f'../plots/emcee_H0_Om0_seed_{seed}.png')
