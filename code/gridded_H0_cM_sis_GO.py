...
def Loglik(free_param, dL_mean, dL_std, t_delay_obs, sigma_t_sq,zL,zS,thetaE,yh):
    ...
    # if 10<=H0<=200 and 0.05<=Om0<=0.95:
    cosmo = FlatLambdaCDM(H0=H0,Om0=Om0)
    dGW_dL_ratio = dGW_dL_ratio_cM(zS,cM,Om0)
    loglik += -0.5*(dL_mean - dGW_dL_ratio*cosmo.luminosity_distance(zS).value)**2/dL_std**2
...
#         break
    ...
    for j in range(len(lensed_index)):
        ...
        cM_arr = np.linspace(-10,10,100)
        loglik = np.zeros((len(h0_arr),len(cM_arr)))
        comb_likelihood = np.ones((len(h0_arr),len(cM_arr)))
        for i in range(len(h0_arr)):
            # for j in range(len(Om0_arr)):
            for k in range(len(cM_arr)):
                loglik[i,k] = Loglik([h0_arr[i],cM_arr[k]], dL_mean, dL_std, t_delay_obs, sigma_t_sq,zL,zS,thetaE,yh)
        np.savetxt(f'../likelihood/H0_cM_loglik_seed_{seed}_ind_{ind}.txt',loglik)
        # lik = np.load('H0_lambda_loglik_seed_0_ind_5081.npz')['arr_0']
        comb_likelihood *= np.exp(loglik)

    np.savetxt(f'../likelihood/H0_cM_loglik_seed_{seed}_combine.txt',comb_likelihood)
    # comb_likelihood = np.load(f'../likelihood/H0_Om0_loglik_seed_{seed}_combine.txt')['arr_0']

    ndim = 2
    param_values = [h0_arr,cM_arr]
    label = [r'$H_0$',r'$c_M$']
    ...
    for column in np.arange(0,ndim):
        for row in np.arange(0,ndim):
            ...
            if column > row:
                ...
                if row==0:
                    ax[row,column].set_ylim(np.min(ind_lik_norm))
                else:
                    ax[row,column].set_ylim(np.min(ind_lik_norm),1.2*np.max(ind_lik_norm))
                ...
                print('%.2f + %.2f - %.2f'%(con_int.map, con_int.upper_level-con_int.map, con_int.map-con_int.lower_level))

                ax[row,row].set_xlim(param_values[row][0],param_values[row][-1])
                # ax[1,1].set_xlim(param_values[1][0],param_values[1][-1])
                # ax[2,2].set_xlim(param_values[2][0],param_values[2][-1])
                ...
                ax[row,column].contourf(param_values[column],param_values[row],comb_likelihood_2d/np.max(comb_likelihood_2d),levels=(np.exp(-4.5), np.exp(-2),np.exp(-0.5),1),cmap='Blues')
                ax[row,column].set_xlim(param_values[column][0],param_values[column][-1])
                ax[row,column].set_ylim(param_values[row][0],param_values[row][-1])
                ax[row,column].xaxis.set_tick_params(labelsize=15)
                ax[row,column].yaxis.set_tick_params(labelsize=15)
            ...
            ax[1,1].axvline(0,color='grey',linestyle='--')
            # ax[2,2].axvline(0,color='grey',linestyle='--')

    plt.savefig(f'../plots/lik_h0_cM_combine_seed_{seed}.png')
