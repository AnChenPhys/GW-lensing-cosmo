...
def t_delay(H0,Om0,zL,zS,thetaE,y,w0,wa):
    cosmo_H0 = w0waCDM(H0=H0,Om0=Om0,Ode0=1-Om0,w0=w0,wa=wa)
...
def log_likelihood(free_param,dL_mean,snrs,t_delay_obs,zL,zS,thetaE,yh):
    H0 = free_param[0]
    Om0 = free_param[1]
    w0 = free_param[2]
    wa = free_param[3]
    ...
    loglik = 0
    if 10<=H0<=200 and 0.01<=Om0<=0.99 and -3<=w0<=0 and -5<=wa<=5:
        cosmo = w0waCDM(H0=H0,Om0=Om0,Ode0=1-Om0,w0=w0,wa=wa)
        dL_std = 0.2*dL_mean*11/snrs

        for i in range(len(dL_mean)):
            loglik += -0.5 * (dL_mean[i] - cosmo.luminosity_distance(zS[i]).value)**2/dL_std[i]**2

            t_delay_thy = t_delay(H0,Om0,zL[i],zS[i],thetaE[i],yh[i],w0,wa)
            loglik += -0.5 * (t_delay_obs[i] - t_delay_thy)**2/(0.25*t_delay_obs[i])**2
    else:
        loglik = -np.inf

    return loglik

fr = open("../XG_Tobs5_snr11/GW_injections_XG.p", "rb")
data = pickle.load(fr)
fr.close()
...
#         break
    ...
    snrs = np.zeros(len(lensed_index))
    for j in range(len(lensed_index)):
        ind = lensed_index[j]

        dL_mean[j] = data['injections_parameters']['dls'][ind]
        snrs[j] = data['injections_parameters']['snrs'][ind]
    ...
    nwalkers = 32
    ndim = 4
    p0 = np.random.rand(nwalkers, ndim)*np.array([190,0.9,3,10]) + np.array([10,0.5,-3,-5])

    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_likelihood, args=[dL_mean, snrs, t_delay_obs,zL_ind,zS_ind,thetaE,y_ind])
    ...
    np.savetxt(f'../emcee_samples_XG/emcee_H0_Om0_w0wa_seed_{seed}_samples_XG.txt',samples)
    ...
    plt.savefig(f'../plots_emcee_XG/emcee_H0_Om0_w0wa_seed_{seed}_XG.png')
