...
class GW_likelihood(bilby.Likelihood):
    def __init__(self,free_param,F_h_model,waveform_generator,psd_interp_list,f_low):
        self.free_param = free_param
        self.F_h_model = F_h_model
    ...
    def log_likelihood(self):
        ...
        F_h_0 = (h0['plus']+h0['cross']) * np.exp(-2.j*np.pi*freq0*self.parameters['tc'])#*np.conjugate(F_0)
            
        ind = np.where(freq0>self.f_low)[0]
        diff = F_h_0[ind]-self.F_h_model[ind]

        loglike = 0
        for psd_interp in self.psd_interp_list:
            loglike -= 0.5*scalar_product(diff,diff,psd_interp(freq0[ind]),freq0[ind])
...
psd_interp_list = [ligo_interp,virgo_interp,kagra_interp]
...
free_param = ['chirp_mass','q','luminosity_distance','theta_jn','geocent_time']#,'logML','y']
...
priors['theta_jn'] = bilby.core.prior.Sine(minimum=0, maximum=np.pi,latex_label=r'$\iota$')
priors['tc'] = bilby.prior.Uniform(-10,10,latex_label=r'$t_c$')
# priors['logML'] = bilby.prior.Uniform(1.,10.,latex_label=r'$\log_{10} M_L$')
# priors['y'] = bilby.prior.Uniform(0.01,10.,latex_label=r'$y$')

likelihood=GW_likelihood(free_param,F_h_0_GO,waveform_generator,psd_interp_list,f_low)
...
result=bilby.run_sampler(likelihood, priors, sampler='nessai',nlive=700,naccept=60, check_point_delta_t=1800, print_method='interval-60', 
                         sample='acceptance-walk', npool=16,outdir='./results_test/',allow_multi_valued_likelihood = True)
