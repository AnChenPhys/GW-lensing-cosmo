...
#         break
    ...
    for j in range(len(lensed_index)):
        ...
        loglik = np.zeros((len(h0_arr),len(Om0_arr),len(cM_arr)))
        comb_likelihood = np.ones((len(h0_arr),len(Om0_arr),len(cM_arr)))
        for i in range(len(h0_arr)):
            for j in range(len(Om0_arr)):
                for k in range(len(cM_arr)):
                    loglik[i,j,k] = Loglik([h0_arr[i],Om0_arr[j],cM_arr[k]], dL_mean, dL_std, t_delay_obs, sigma_t_sq,zL,zS,thetaE,yh)
        np.savez(f'../likelihood/H0_Om0_cM_loglik_seed_{seed}_ind_{ind}.npz',loglik)
        # lik = np.load('H0_lambda_loglik_seed_0_ind_5081.npz')['arr_0']
        comb_likelihood *= np.exp(loglik)

    np.savez(f'../likelihood/H0_Om0_cM_loglik_seed_{seed}_combine.npz',comb_likelihood)
    ...
    fig, ax = plt.subplots(ndim, ndim, figsize=[15,13],constrained_layout=True)
    ...
    plt.savefig(f'../plots/lik_h0_Om0_cM_combine_seed_{seed}.png')
